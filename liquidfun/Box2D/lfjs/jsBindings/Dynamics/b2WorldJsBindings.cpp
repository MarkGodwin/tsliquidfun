...
extern "C" {
    extern void b2WorldBeginContactBody(void *worldPtr, void* contactPtr);
    extern void b2WorldEndContactBody(void *worldPtr, void* contactPtr);
    extern void b2WorldPreSolve(void *worldPtr, void* contactPtr, void* oldManifoldPtr);
    extern void b2WorldPostSolve(void *worldPtr, void* contactPtr, void* impulsePtr);
    extern bool b2WorldQueryAABB(void *worldPtr, void* fixturePtr);
    extern float32 b2WorldRayCastCallback(void *worldPtr, void* fixturePtr, double pointX, double pointY,
...
{
public:
    b2WorldContactListener(b2World *world) : _world(world) { }

private:
    b2World *_world;

    void BeginContact(b2Contact* contact) {
        b2WorldBeginContactBody(_world, contact);
    }

    void EndContact(b2Contact* contact) {
        b2WorldEndContactBody(_world, contact);
    }

    void PreSolve(b2Contact* contact, const b2Manifold* oldManifold) {
        b2WorldPreSolve(_world, contact, const_cast<b2Manifold*>(oldManifold));
    }

    void PostSolve(b2Contact* contact, const b2ContactImpulse* impulse) {
        b2WorldPostSolve(_world, contact, const_cast<b2ContactImpulse*>(impulse));
    }
...
public:
    QueryAABBCallback(b2World *world) : _world(world) { }

    bool ReportFixture(b2Fixture* fixture) {
        return b2WorldQueryAABB(_world, (void*)fixture);
    }
...
public:
    RayCastCallback(b2World *world) : _world(world) { }

    float32 ReportFixture(b2Fixture* fixture, const b2Vec2& point,
        const b2Vec2& normal, float32 fraction) {
        return b2WorldRayCastCallback(_world, fixture, point.x, point.y,
...
void b2World_QueryAABB(void* world, double aabbLowerBoundX, double aabbLowerBoundY,
    ...
    QueryAABBCallback cb((b2World*)world);
    ((b2World*)world)->QueryAABB(&cb, aabb);
}

void b2World_RayCast(void* world, double point1X, double point1Y,
    double point2X, double point2Y) {
    RayCastCallback cb((b2World *)world);
    ((b2World*)world)->RayCast(&cb, b2Vec2(point1X, point1Y),
        b2Vec2(point2X, point2Y));
}

void b2World_SetContactListener(void* world) {
    b2World *theWorld = ((b2World*)world);
    theWorld->SetContactListener(new b2WorldContactListener(theWorld)); // Yes, we're going to leak this
}
